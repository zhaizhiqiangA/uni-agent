...
`mini_swe` run inside the SWE-bench sandbox through a sidecar tool image. 
The external runner creates the sandbox, mounts the selected
...
The OpenYuanrong Sandbox Service
'openyuanrong' is the AKernel sandbox service, managing and scheduling sandbox resources, built on top of Kubernetes cluster.
...
[Rollouter Host: mini_swe_agent_runner]
  ...
  |     `-- akernel: Sandbox(mounts=[Mount(target="/opt/<tool>", ...)])
  |
...
## 2. Inference With AKernel Sandbox
