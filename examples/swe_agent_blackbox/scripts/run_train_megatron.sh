#!/usr/bin/env bash
# Training launch script for the blackbox SWE-agent recipe (Megatron backend).
#
# Uses GRPO + AgentFrameworkRolloutAdapter with Megatron-based actor/critic.
# Otherwise identical to run_train.sh — same reward path, same agent framework.
#
# Usage:
#   bash examples/swe_agent_blackbox/scripts/run_train_megatron.sh
#
...
EXPERIMENT_NAME="${EXPERIMENT_NAME:-swe_agent_megatron_$(date +%Y%m%d_%H%M)}"
...
echo "=== SWE-Agent Blackbox Training (Megatron) ==="
echo "Model:       ${MODEL_PATH}"
echo "Train data:  ${TRAIN_DATA}"
echo "Val data:    ${VAL_DATA}"
echo "Engine:      ${ENGINE} (TP=${TP})"
echo "Megatron:    TP=${MEGATRON_TP}, PP=${MEGATRON_PP}, CP=${MEGATRON_CP}"
echo "Batch size:  ${TRAIN_BATCH_SIZE}, N=${N}"
echo "Epochs:      ${TOTAL_EPOCHS}"
echo "=============================================="

python3 -m verl.trainer.main_ppo_sync \
    --config-name=swe_agent_blackbox_megatron \
