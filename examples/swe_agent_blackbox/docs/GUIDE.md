...
pip install --no-deps git+https://github.com/ISEEKYAN/mbridge.git
```

Megatron 特有参数：
```bash
MEGATRON_TP=8    # Megatron actor tensor parallel
MEGATRON_PP=1    # Megatron actor pipeline parallel
MEGATRON_CP=1    # Megatron actor context parallel
```
...
FSDP 配置：`examples/swe_agent_blackbox/config/swe_agent_blackbox.yaml`
Megatron 配置：`examples/swe_agent_blackbox/config/swe_agent_blackbox_megatron.yaml`
