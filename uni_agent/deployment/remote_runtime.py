...
class RemoteRuntime(AbstractRuntime):
    ...
    @staticmethod
    def _unverified_tls12_ssl_context() -> ssl.SSLContext:
        """Match curl -k + TLS1.2 for openyuanrong gateways (connector-level ssl)."""
    ...
    def _make_connector(self) -> aiohttp.TCPConnector:
        if self._api_url.startswith("https://") and not self._config.ssl_verify:
            return aiohttp.TCPConnector(ssl=self._unverified_tls12_ssl_context(), force_close=True)
