...
class TestComputeScore:
    ...
    def test_returns_score_from_extra_info(self):
        ...
        assert compute_score("swe_bench", "", "", extra_info={"reward_score": 1.0}) == 1.0
        assert compute_score("swe_bench", "", "", extra_info={"reward_score": 0.0}) == 0.0

    def test_returns_zero_when_no_extra_info(self):
        from examples.swe_agent_blackbox.reward import compute_score

        assert compute_score("swe_bench", "", "") == 0.0
        assert compute_score("swe_bench", "", "", extra_info=None) == 0.0

    def test_returns_zero_when_no_reward_score_key(self):
        from examples.swe_agent_blackbox.reward import compute_score

        assert compute_score("swe_bench", "", "", extra_info={"other": 1.0}) == 0.0
...
class TestToolParserDetection:
    """VERIFICATION #30: _detect_tool_parser covers Qwen3 / Hermes."""

    def test_qwen3_coder(self):
        from examples.swe_agent_blackbox.parallel_infer import _detect_tool_parser

        with tempfile.TemporaryDirectory() as tmpdir:
            with open(os.path.join(tmpdir, "tokenizer_config.json"), "w") as f:
                json.dump({"chat_template": "{% if tools %}<function={{}}><parameter={{}}>{% endif %}"}, f)
            assert _detect_tool_parser(tmpdir) == "qwen3_coder"
    ...
    def test_hermes(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            with open(os.path.join(tmpdir, "tokenizer_config.json"), "w") as f:
                json.dump({"chat_template": '{% if tools %}{{ "name" }}{% endif %}'}, f)
            assert _detect_tool_parser(tmpdir) == "hermes"
    ...
    def test_no_tools_returns_none(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            with open(os.path.join(tmpdir, "tokenizer_config.json"), "w") as f:
                json.dump({"chat_template": "plain template"}, f)
            assert _detect_tool_parser(tmpdir) is None
...
class TestToolParserRealModel:
    """Tool parser detection with real Qwen3.5-4B tokenizer."""

    def test_qwen35_4b_detects_parser(self):
        from examples.swe_agent_blackbox.parallel_infer import _detect_tool_parser

        result = _detect_tool_parser("/data1/models/Qwen/Qwen3.5-4B")
...
# =====================================================================
# Tier 4 — Training end-to-end (VERIFICATION §2.1, multi-GPU required)
# =====================================================================
...
class TestTrainingEndToEnd:
    """VERIFICATION #33–35: training end-to-end (manual / CI-only)."""

    def test_train_1_epoch_swe_bench(self):
        """Run 1 epoch training on swe_bench sample data.

        This test is expensive and typically run manually:
            pytest tests/test_swe_agent_blackbox.py::TestTrainingEndToEnd -v --run-training
        """
        pytest.skip("Training test — run manually with --run-training flag")

    def test_train_reward_curve(self):  # #34
        pytest.skip("Requires tensorboard inspection after training run")

    def test_train_checkpoint_save_load(self):  # #35
        pytest.skip("Requires training run + checkpoint verification")
